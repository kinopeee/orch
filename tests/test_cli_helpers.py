...
def test_render_plan_error_sanitizes_symbolic_link_tab_separated_detail() -> None:
    err = PlanError("plan path has symbolic\tlink reference: /tmp/plan.yaml")
    assert _render_plan_error(err) == "invalid plan path"
...
def test_render_plan_error_sanitizes_symbolic_link_form_feed_separated_detail() -> None:
...
def test_render_plan_error_sanitizes_symbolic_links_mixed_separator_uppercase_detail() -> None:
    err = PlanError("PLAN PATH HAS SYMBOLIC_-LINKS REFERENCE: /TMP/PLAN.YAML")
    assert _render_plan_error(err) == "invalid plan path"


def test_render_plan_error_sanitizes_symbolic_link_double_hyphen_separator_detail() -> None:
    err = PlanError("plan path has symbolic--link reference: /tmp/plan.yaml")
    assert _render_plan_error(err) == "invalid plan path"


def test_render_plan_error_sanitizes_symbolic_linking_hyphenated_detail() -> None:
    err = PlanError("plan path has symbolic-linking issue: /tmp/plan.yaml")
    assert _render_plan_error(err) == "invalid plan path"


def test_render_plan_error_sanitizes_symbolic_links_tab_separated_detail() -> None:
    err = PlanError("plan path has symbolic\tlinks reference: /tmp/plan.yaml")
    assert _render_plan_error(err) == "invalid plan path"


def test_render_plan_error_sanitizes_symbolic_links_newline_separated_detail() -> None:
    err = PlanError("plan path has symbolic\nlinks reference: /tmp/plan.yaml")
    assert _render_plan_error(err) == "invalid plan path"
...
def test_render_plan_error_sanitizes_symbolically_linked_double_hyphen_detail() -> None:
    err = PlanError("plan path is symbolically--linked to another location")
    assert _render_plan_error(err) == "invalid plan path"
...
def test_render_plan_error_sanitizes_symbolically_links_plural_detail() -> None:
...
def test_render_runtime_error_detail_sanitizes_symbolic_link_tab_separated_detail() -> None:
    err = OSError("run path has symbolic\tlink reference")
    assert _render_runtime_error_detail(err) == "invalid run path"
...
def test_render_runtime_error_detail_sanitizes_symbolic_link_form_feed_separated_detail() -> None:
    err = OSError("run path has symbolic\flink reference")
    assert _render_runtime_error_detail(err) == "invalid run path"


def test_render_runtime_error_detail_sanitizes_symbolic_links_vertical_tab_separated_detail() -> (
    None
):
...
def test_render_runtime_error_detail_sanitizes_symbolic_link_mixed_separator_uppercase_detail() -> (
    None
):
...
def test_render_runtime_error_detail_sanitizes_symbolic_links_double_underscore_detail() -> None:
    err = OSError("run path has symbolic__links reference")
    assert _render_runtime_error_detail(err) == "invalid run path"


def test_render_runtime_error_detail_sanitizes_symbolic_linking_underscored_detail() -> None:
    err = OSError("run path has symbolic_linking issue")
    assert _render_runtime_error_detail(err) == "invalid run path"


def test_render_runtime_error_detail_sanitizes_symbolic_links_tab_separated_detail() -> None:
    err = OSError("run path has symbolic\tlinks reference")
    assert _render_runtime_error_detail(err) == "invalid run path"


def test_render_runtime_error_detail_sanitizes_symbolic_links_newline_separated_detail() -> None:
    err = OSError("run path has symbolic\nlinks reference")
    assert _render_runtime_error_detail(err) == "invalid run path"
...
def test_render_runtime_error_detail_sanitizes_symbolically_linked_double_underscore_detail() -> (
    None
):
...
""".strip(),
    ...
    def boom_initialize(_run_dir: Path) -> None:
        raise OSError("layout path has symbolic-linkers issue")
...
def test_cli_run_keeps_symbolic_linkingly_execution_error_detail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    plan_path = tmp_path / "plan.yaml"
    plan_path.write_text(
...
def test_cli_run_sanitizes_symbolically_underscore_linked_plan_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    plan_path = tmp_path / "plan.yaml"
    plan_path.write_text("tasks: []\n", encoding="utf-8")
...
def test_cli_run_sanitizes_symbolically_linking_double_underscore_plan_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    plan_path = tmp_path / "plan.yaml"
    plan_path.write_text("tasks: []\n", encoding="utf-8")
    ...
    def fake_load_plan(_path: Path) -> PlanSpec:
        raise PlanError("plan path has symbolically__linking issue")
...
def test_cli_run_keeps_symbolic_linker_plan_error_detail(
    ...
    def fake_load_plan(_path: Path) -> PlanSpec:
        raise PlanError("plan path has symbolic-linker issue")
...
def test_cli_run_keeps_symbolic_linkless_plan_error_detail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    plan_path = tmp_path / "plan.yaml"
    plan_path.write_text("tasks: []\n", encoding="utf-8")
    ...
    def fake_load_plan(_path: Path) -> PlanSpec:
        raise PlanError("plan path has symbolic_linkless issue")
...
def test_cli_run_keeps_symbolically_linkingly_double_underscore_plan_error_detail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    plan_path = tmp_path / "plan.yaml"
    plan_path.write_text("tasks: []\n", encoding="utf-8")
    ...
    def fake_load_plan(_path: Path) -> PlanSpec:
        raise PlanError("plan path has symbolically__linkingly issue")
...
def test_cli_resume_sanitizes_symbolically_underscore_linked_conflict_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
    ...
    @contextmanager
    def boom_conflict(*args: object, **kwargs: object) -> object:
        raise RunConflictError("run lock path is symbolically_linked to another location")
        yield
...
def test_cli_resume_keeps_symbolically_linkingly_double_underscore_conflict_error_detail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
    ...
    @contextmanager
    def boom_conflict(*args: object, **kwargs: object) -> object:
        raise RunConflictError("run lock path has symbolically__linkingly issue")
        yield
...
def test_cli_status_sanitizes_symbolically_linking_runtime_load_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
...
def test_cli_status_sanitizes_symbolically_underscore_linked_runtime_load_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
    ...
    def boom_load_state(_run_dir: Path) -> object:
        raise OSError("run path is symbolically_linked to another location")
...
def test_cli_status_keeps_symbolic_linkless_runtime_load_error_detail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
...
def test_cli_status_keeps_symbolically_linkingly_double_underscore_runtime_load_error_detail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
...
def test_cli_logs_sanitizes_symbolically_linking_runtime_load_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
...
def test_cli_logs_sanitizes_symbolically_underscore_linked_runtime_load_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
...
def test_cli_cancel_sanitizes_symbolically_linking_write_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    run_dir = home / "runs" / "run1"
...
def test_cli_cancel_sanitizes_symbolically_underscore_linked_write_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    run_dir = home / "runs" / "run1"
    ...
    def boom_write_cancel(_run_dir: Path) -> None:
        raise OSError("cancel request path is symbolically_linked to another location")
...
def test_cli_cancel_sanitizes_symbolically_linking_double_underscore_write_error(
    ...
    def boom_write_cancel(_run_dir: Path) -> None:
        raise OSError("cancel request path has symbolically__linking issue")
...
def test_cli_cancel_keeps_symbolically_linkingly_double_underscore_write_error_detail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    run_dir = home / "runs" / "run1"
    ...
    def boom_write_cancel(_run_dir: Path) -> None:
        raise OSError("cancel request path has symbolically__linkingly issue")
...
def test_cli_resume_sanitizes_symbolically_linking_double_underscore_plan_error(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
...
def test_cli_resume_keeps_symbolic_linkless_plan_error_detail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
...
def test_cli_resume_keeps_symbolic_linker_plan_error_detail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
...
def test_cli_resume_keeps_symbolically_linkingly_double_underscore_plan_error_detail(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
...
def test_cli_run_sanitizes_symbolically_linking_double_underscore_report_warning(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    plan_path = tmp_path / "plan.yaml"
    plan_path.write_text(
...
""".strip(),
    ...
    def boom_write_report(_state: RunState, _run_dir: Path) -> Path:
        raise OSError("report path has symbolically__linking issue")
...
""".strip(),
    ...
    def boom_write_report(_state: RunState, _run_dir: Path) -> Path:
        raise OSError("report path has symbolically_linkedness issue")
...
def test_cli_run_keeps_symbolically_linkingly_double_underscore_report_warning(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    plan_path = tmp_path / "plan.yaml"
    plan_path.write_text(
...
""".strip(),
    ...
    def boom_write_report(_state: RunState, _run_dir: Path) -> Path:
        raise OSError("report path has symbolically__linkingly issue")
...
def test_cli_resume_sanitizes_symbolically_linking_double_underscore_report_warning(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
...
def test_cli_resume_keeps_symbolic_linkers_report_write_warning_detail(
    ...
    def boom_write_report(_state: RunState, _run_dir: Path) -> Path:
        raise OSError("report path has symbolic-linkers issue")
...
def test_cli_resume_keeps_symbolically_linkingly_double_underscore_report_warning(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]
) -> None:
    home = tmp_path / ".orch"
    home.mkdir()
